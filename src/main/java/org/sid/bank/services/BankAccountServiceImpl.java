...
import org.sid.bank.mappers.BankAccountMapperImpl;
...
public class BankAccountServiceImpl implements BankAccountService {
    ...
    public List<BankAccountDTO>bankAccountList(){
        ...
        }).collect(Collectors.toList());return bankAccountDTOS;
    }
    @Override
    ...
    @Override
    public AccountHistoryDTO getAccountHistory(String accountId, int page, int size) throws BankAccountNotFoundException {BankAccount bankAccount=bankAccountRepository.findById(accountId).orElse(null);
        if(bankAccount==null)throw new BankAccountNotFoundException("Account not found");
        Page<AccountOperation> accountOperations = accountOperationRepository.findByBankAccountId(accountId,PageRequest.of(page,size));
        AccountHistoryDTO accountHistoryDTO=new AccountHistoryDTO();
        List<AccountOperationDTO> accountOperationDTOS =accountOperations.getContent().stream().map(op->DTOMapper.fromAccountOperation(op)).collect(Collectors.toList());
        accountHistoryDTO.setAccountsOperationDTOS(accountOperationDTOS);
